...
type Variant = "danger" | "success" | "primary" | "secondary" | "ghost";
...
interface Props {
  ...
  primaryVariant?: Exclude<Variant, "secondary" | "ghost">;
...
export default function Modal({
  ...
  useEffect(() => {
    if (!open) return;
    const onKeyCapture = (e: KeyboardEvent) => { if (e.key === "Escape") { e.stopPropagation(); safeClose(); } };
  ...
  useEffect(() => { if (open) (dialogRef.current as HTMLElement | null)?.focus(); }, [open]);

  const hasActions = useMemo(() => Boolean(primaryText) || Boolean(secondaryText), [primaryText, secondaryText]);
  if (!open) return null;

  const handleCardKeyDown = (e: React.KeyboardEvent) => { if (e.key === "Escape") { e.stopPropagation(); safeClose(); } };
  const handleBackdropMouseDown = (e: React.MouseEvent<HTMLDivElement>) => { if (!closeOnOverlay) return; if (e.target === overlayRef.current) { safeClose(); } };
